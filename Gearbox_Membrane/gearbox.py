...
async def verify_state_machine(request: ProofOfWorkRequest, api_key_hash: str = Security(verify_access)):
    ...
    if request.task_type == "python_code":
        ...
        try:
            ...
            if has_export and has_return and brace_diff == 0:
                payload_hash = hashlib.sha256(payload.encode()).hexdigest()
                signature = f"MEMBRANE_VERIFIED_{payload_hash[:16]}"
                return ProofOfWorkResponse(verified=True, membrane_signature=signature)
            else:
                errs = []
                if not has_export: errs.append("Missing 'export default'")
                if not has_return: errs.append("Missing return statement")
                if brace_diff != 0: errs.append(f"Brace mismatch (difference: {brace_diff})")
                return ProofOfWorkResponse(verified=False, error_log=f"Lightweight compile failed: {', '.join(errs)}")
        except Exception as e:
            return ProofOfWorkResponse(verified=False, error_log=f"Lightweight parsing exception: {str(e)}")
