...
import { isProductionRuntime } from "@/lib/admin-auth";
...
export async function POST(req: Request) {
  ...
  const endpointSecret = process.env.STRIPE_WEBHOOK_SECRET || process.env.POLAR_WEBHOOK_SECRET || "";

  if (isProd && !endpointSecret) {
    return new NextResponse("Webhook secret is not configured", { status: 503 });
  }

  // Validate webhook signatures in production. Local development can use unsigned sample events.
  if (isProd) {
    if (!signature) {
    ...
    if (signature.includes("t=") && signature.includes("v1=")) {
      ...
      if (!safeSignatureEqual(stripeHmac, v1Sig)) {
      ...
      const match = safeSignatureEqual(digest, normalizedSignature);
      if (!match) {
