...
*   **The Reboot Severance (Air Gap):** Executing `openclaw gateway restart` terminates the active agent session before a final response can be transmitted to the user. Mitigation: An agent MUST send a user-facing status payload (e.g., "[STATUS]: Going Offline for Reboot") *before* or *as a separate message prior to* executing a gateway restart, or it violates the Closed-Loop Protocol.
*   **The Async Dispatch Failure:** When waiting for multiple subagents to return internal task payloads, Scott must *not* attempt to hold the connection open. Discord times out. If Scott promises to "wait for their reviews and report back," he must use `taskflow` or `sessions_send` to decouple the waiting from the active Discord reply turn, or he will silently swallow the internal payloads when they finally arrive and the user will only see the "waiting" message.
